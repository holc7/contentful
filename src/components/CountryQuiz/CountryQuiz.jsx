  ...
  const handleNextQuestion = () => {
    if (feedback === '') {
      if (selectedOption === shuffledQuestions[currentQuestionIndex].answer) {
        ...
        setFeedback('Incorrect. The correct answer was: ' + shuffledQuestions[currentQuestionIndex].answer);
      }
      ...
      {currentQuestionIndex < shuffledQuestions.length ? (
        <>
          ...
          {shuffledQuestions[currentQuestionIndex].options.map((option) => (
            <button key={option} onClick={() => handleOptionClick(option)} disabled={feedback !== '' || selectedOption === option}>
              {option}
            </button>
          ))}
          <div>
